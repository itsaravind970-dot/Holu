...
import { 
  ...
  Menu, X, Sparkles, User, AlertTriangle, Loader2, Cpu, History, BookMarked, Copy, Trash2, ArrowLeft, CheckCircle2, Lock, Smartphone, UserCircle, LogOut, Eye, EyeOff, Database, ShieldCheck, Activity, Fingerprint, Globe, ShieldAlert, Ban, CheckCircle, Shield, Info, RefreshCcw, Zap, Camera, SearchCode
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (text: string, file?: { data: string; mimeType: string }) => {
    ...
    if (!activeId) {
      const newS: ChatSessionHistory = { id: Date.now().toString(), title: text.slice(0, 25) || "Uplink", messages: [], updatedAt: Date.now() };
    ...
    } catch (e) { console.error(e); } finally { setIsLoading(false); abortControllerRef.current = null; }
  };
  ...
  const sharedUI = (
    <>
      {/* RESTORED ELITE ADMIN PANEL */}
      {showAdminPanel && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/98 p-0 sm:p-6 md:p-10 animate-in fade-in duration-300">
          <div className="bg-[#0b0f19] w-full h-full sm:rounded-[32px] md:rounded-[48px] border border-slate-800 flex flex-col overflow-hidden shadow-2xl">
            <div className="p-5 md:p-8 border-b border-slate-800 flex items-center justify-between bg-slate-900/40">
              <div className="flex items-center gap-3 md:gap-6">
                ...
                <div>
                  <h2 className="text-lg md:text-2xl font-black uppercase tracking-tight text-white leading-tight">Security Command</h2>
                  <p className="text-[8px] md:text-[10px] font-black text-green-500 uppercase tracking-widest mt-0.5">MASTER PORTAL ACCESS</p>
                </div>
              </div>
              <button onClick={() => setShowAdminPanel(false)} className="p-3 md:p-4 bg-slate-800 text-slate-400 rounded-xl md:rounded-2xl hover:text-white transition-all"><X size={20} className="md:w-6 md:h-6" /></button>
            </div>
                    ...
                    <div className="flex justify-between items-start mb-4 md:mb-6">
                      <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="text-white font-bold text-sm md:text-base truncate max-w-[120px]">{acc.username}</p>
                          <p className="text-[8px] md:text-[10px] text-slate-500 uppercase tracking-tighter mt-0.5">{acc.phoneNumber}</p>
                        </div>
                      </div>
                      <div className="flex gap-1.5 md:gap-2">
                        ...
                        <button onClick={() => { if(confirm('Purge node?')) {
                          const updated = adminAccounts.filter(a => a.id !== acc.id);
                          localStorage.setItem('hulu_accounts', JSON.stringify(updated));
                          refreshAdminData();
                        }}} className="p-2 bg-slate-800 text-slate-400 rounded-xl hover:bg-red-600 hover:text-white"><Trash2 size={14} /></button>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="bg-black/40 p-3 md:p-4 rounded-2xl border border-slate-800/50 flex justify-between items-center">
                        <span className="text-[8px] md:text-[9px] text-slate-500 uppercase font-black">Passkey</span>
                        <span className="text-xs md:text-sm font-mono text-green-400 font-bold tracking-widest">{acc.password}</span>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {/* RESTORED ELITE PROFILE OVERLAY */}
      {showProfile && currentUser && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 bg-slate-900/90 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white rounded-[32px] md:rounded-[48px] w-full max-w-md overflow-hidden shadow-2xl border border-white animate-in zoom-in-95 duration-300">
             <div className="p-6 md:p-10 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="relative group">
                    ...
                    <button onClick={() => profilePicInputRef.current?.click()} className="absolute bottom-0 right-0 bg-slate-900 text-white p-2 rounded-full border-2 border-white shadow-lg"><Camera size={12} /></button>
                    <input type="file" ref={profilePicInputRef} className="hidden" accept="image/*" onChange={(e) => {
                      const f = e.target.files?.[0];
                      if (f) {
                        const r = new FileReader();
                        r.onloadend = () => {
                          const u = { ...currentUser, profilePic: r.result as string };
                          setCurrentUser(u);
                        };
                        r.readAsDataURL(f);
                      }
                  ...
                  <div>
                    <h3 className="font-black text-xl md:text-2xl uppercase tracking-tighter leading-none">{currentUser.username}</h3>
                    <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1.5 md:mt-2">ACTIVE SECURITY NODE</p>
                  </div>
                </div>
                <button onClick={() => setShowProfile(false)} className="p-3 md:p-4 bg-white border border-slate-100 rounded-2xl hover:bg-slate-50 transition-all shadow-sm"><X size={18} className="md:w-5 md:h-5" /></button>
             </div>
             <div className="p-6 md:p-10 space-y-3 md:space-y-4">
                {[
                  ...
                  <div key={idx} className="flex items-center gap-4 p-4 md:p-5 bg-slate-50/80 rounded-[20px] md:rounded-[28px] border border-slate-100/50">
                    <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-xl md:rounded-2xl flex items-center justify-center shadow-sm text-slate-400"><item.icon size={16} className="md:w-5 md:h-5" /></div>
                    <div className="min-w-0 flex-1">
                      <p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">{item.label}</p>
                      <p className="text-xs md:text-sm font-bold text-slate-900 truncate">{item.value}</p>
                    </div>
                  </div>
                ))}
                <button onClick={handleLogout} className="w-full mt-4 py-5 md:py-6 bg-red-500 text-white rounded-[24px] md:rounded-[28px] text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] shadow-xl shadow-red-100 hover:bg-red-600 transition-all flex items-center justify-center gap-2"><LogOut size={16} /> TERMINATE UPLINK</button>
             </div>
          </div>
        </div>
      )}
  ...
  if (isAuthView) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        {sharedUI}
        <AuthScreen onLogin={(user) => { setCurrentUser(user); setIsAuthView(false); loadUserData(user.id); }} />
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-white text-slate-900 overflow-hidden font-jakarta">
      ...
      {/* RESPONSIVE SIDEBAR */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 md:w-80 bg-slate-50 border-r border-slate-200 transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="p-5 md:p-8">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 md:w-10 md:h-10 bg-slate-900 rounded-full flex items-center justify-center text-white overflow-hidden shadow-lg">
                  {currentUser?.profilePic ? <img src={currentUser.profilePic} className="w-full h-full object-cover" /> : <Cpu size={16} className="md:w-5 md:h-5" />}
                </div>
            ...
            <div className="relative mb-4 md:mb-6">
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search nodes..." className="w-full bg-white border border-slate-200 rounded-[18px] md:rounded-[24px] py-2.5 md:py-3.5 pl-10 pr-4 text-[11px] md:text-[12px] font-bold outline-none focus:ring-4 focus:ring-slate-100 transition-all" />
            </div>

            <button onClick={() => { setCurrentSessionId(null); setSelectedProjectId(null); setIsSidebarOpen(false); setView('chats'); }} className="w-full bg-slate-900 text-white py-4 md:py-5 rounded-[20px] md:rounded-[24px] text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-black transition-all active:scale-[0.98] flex items-center justify-center gap-2"><Plus size={18} /> New Session</button>
          </div>

          <div className="flex-1 overflow-y-auto custom-scrollbar px-4 md:px-6 pb-6 space-y-1.5">
            {(view === 'chats' ? filteredSessions : filteredProjects).map((item: any) => (
              <button key={item.id} onClick={() => { 
                ...
                <div className={`p-2 rounded-xl ${((view === 'chats' && currentSessionId === item.id) || (view === 'projects' && selectedProjectId === item.id)) ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'}`}>
                  {view === 'chats' ? <MessageSquare size={14} className="md:w-4 md:h-4" /> : <BookMarked size={14} className="md:w-4 md:h-4" />}
                </div>
        ...
        <header className="h-16 md:h-20 flex items-center justify-between px-5 md:px-10 border-b border-slate-100 sticky top-0 bg-white/90 backdrop-blur-xl z-20">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
               <h2 className="text-xs md:text-sm font-black uppercase tracking-tighter text-slate-900">SECURE NODE: ACTIVE</h2>
          ...
          <button onClick={() => setShowProfile(true)} className="flex items-center gap-3 hover:opacity-80 transition-all active:scale-95 pl-4 border-l border-slate-100">
             <div className="flex flex-col items-end hidden sm:flex">
                ...
                <span className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-1">Profile Detail</span>
             </div>
             <div className="w-9 h-9 md:w-11 md:u-11 rounded-full bg-slate-900 overflow-hidden border-2 border-white shadow-lg">
          ...
          <div className="max-w-4xl mx-auto min-h-full flex flex-col">
            {selectedProjectId ? (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                 ...
                 <div className="bg-white rounded-[32px] md:rounded-[48px] overflow-hidden border border-slate-200 shadow-2xl">
                    <div className="p-8 md:p-12 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                       ...
                       <button onClick={() => navigator.clipboard.writeText(projects.find(p => p.id === selectedProjectId)?.content || '')} className="p-4 md:p-5 bg-slate-900 text-white rounded-2xl md:rounded-[24px] shadow-lg active:scale-95 transition-all"><Copy size={20} /></button>
                    </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center py-10 md:py-20 animate-in fade-in duration-700">
                <div className="w-16 h-16 md:w-24 md:h-24 bg-slate-900 rounded-[20px] md:rounded-[32px] flex items-center justify-center text-white shadow-2xl mb-8 relative overflow-hidden border-2 border-white">
                ...
                <h3 className="text-2xl md:text-5xl font-black text-slate-900 tracking-tighter uppercase mb-2 md:mb-4 text-center">Protocol Standby</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full max-w-2xl px-4">
                  {[`Analyze security feed`, `Synthesize local data`, `Generate code variant`, `Formulate strategy`].map((tip, i) => (
...
const AuthScreen: React.FC<{ onLogin: (u: UserAccount) => void }> = ({ onLogin }) => {
  ...
  const [formData, setFormData] = useState({ username: '', phoneNumber: '', password: '', chatbotName: 'Hulu assis' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      else setError('INVALID SECURITY KEYS.');
    } else {
      if (!formData.username || !formData.phoneNumber || !formData.password) return setError('ALL FIELDS MANDATORY.');
      if (accs.some((a: any) => a.phoneNumber === formData.phoneNumber)) return setError('NODE ALREADY EXISTS.');
  ...
  return (
    <div className="w-full max-w-md animate-in zoom-in-95 duration-700">
      <div className="bg-white p-8 md:p-14 rounded-[48px] md:rounded-[60px] shadow-[0_30px_100px_rgba(0,0,0,0.06)] border border-slate-100">
        <div className="text-center mb-10 md:mb-12">
          <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-900 rounded-[24px] md:rounded-[30px] mx-auto mb-6 flex items-center justify-center text-white shadow-2xl relative">
            <div className="absolute inset-0 bg-green-500/10 animate-pulse"></div>
            <Cpu size={32} className="md:w-10 md:h-10 text-green-400" />
          </div>
          <h2 className="text-2xl md:text-4xl font-black uppercase tracking-tighter">Hulu assis</h2>
          <p className="text-[9px] md:text-[11px] text-slate-400 font-black uppercase tracking-[0.4em] mt-2 md:mt-3">{mode === 'login' ? 'IDENTITY AUTHENTICATION' : 'REGISTRY ENROLLMENT'}</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
          {mode === 'signup' && <div className="relative"><UserCircle className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400" size={18}/><input placeholder="Identity Name" className="w-full bg-slate-50 border border-slate-100 rounded-[20px] md:rounded-[24px] py-4 md:py-5 pl-14 pr-6 text-xs md:text-sm font-bold outline-none focus:ring-8 focus:ring-slate-100 transition-all" value={formData.username} onChange={e => setFormData({ ...formData, username: e.target.value })} /></div>}
          <div className="relative"><Smartphone className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400" size={18}/><input placeholder="Phone Identity" className="w-full bg-slate-50 border border-slate-100 rounded-[20px] md:rounded-[24px] py-4 md:py-5 pl-14 pr-6 text-xs md:text-sm font-bold outline-none focus:ring-8 focus:ring-slate-100 transition-all" value={formData.phoneNumber} onChange={e => setFormData({ ...formData, phoneNumber: e.target.value })} /></div>
          <div className="relative"><Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400" size={18}/><input type="password" placeholder="Access Key" className="w-full bg-slate-50 border border-slate-100 rounded-[20px] md:rounded-[24px] py-4 md:py-5 pl-14 pr-6 text-xs md:text-sm font-bold outline-none focus:ring-8 focus:ring-slate-100 transition-all" value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} /></div>
          {error && <p className="text-[9px] md:text-[10px] font-black text-red-500 uppercase text-center tracking-widest bg-red-50 py-3 rounded-xl">{error}</p>}
          <button type="submit" className="w-full bg-slate-900 text-white py-5 md:py-6 rounded-[20px] md:rounded-[24px] font-black uppercase tracking-[0.3em] text-[10px] md:text-[11px] shadow-2xl active:scale-95 transition-all mt-4">ESTABLISH UPLINK</button>
        </form>
        <button onClick={() => setMode(mode === 'login' ? 'signup' : 'login')} className="w-full text-center mt-8 md:mt-10 text-[9px] md:text-[10px] font-black uppercase text-slate-400 hover:text-black tracking-widest transition-colors">{mode === 'login' ? 'REGISTRY REQUIRED? ENROLL' : 'BACK TO AUTHENTICATION'}</button>
      </div>
    </div>
  );
};
