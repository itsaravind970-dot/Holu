...
const ChatInput: React.FC<Props> = ({ onSend, onStop, disabled }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-2">
      ...
      <div className="flex items-end gap-2 bg-slate-50 rounded-[28px] p-2 border border-slate-200 focus-within:border-slate-400 transition-all">
        ...
        <button type="button" onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-400 hover:text-slate-900 shrink-0"><Paperclip size={20} /></button>
        <textarea
          ref={textareaRef}
          rows={1}
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder={disabled ? "Processing..." : "Message Hulu assis..."}
          ...
          disabled={disabled}
        />
        <button type="button" onClick={handleSubmit} className={`p-3.5 rounded-2xl transition-all shrink-0 ${(!text.trim() && !file) && !disabled ? 'bg-slate-200 text-slate-400' : 'bg-slate-900 text-white shadow-lg active:scale-95'}`}>
